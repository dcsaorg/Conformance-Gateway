...
- `standards-commons/` and `booking-and-ebl/`: shared Booking/eBL scenario state and the combined cross-standard flow.
...
- `lambda/` and `cdk/`: deployed AWS runtime entry points and infrastructure, respectively.
- `webui/`: Angular UI.
...
`specifications/confluence-sync/` is generated by `scripts/src/confluence/sync-confluence.py` and the scheduled `.github/workflows/confluence-sync.yml`; do not hand-edit generated Markdown or workbooks as an implementation fix. Configure synced standards, versions, and pages in `scripts/src/confluence/confluence-config.json` and follow `specifications/SETUP.md` to regenerate them. When changing the sync script or configuration, run both `python3 scripts/src/confluence/sync-confluence.py --self-test` and `python3 -m unittest -v scripts/src/confluence/test_sync_confluence.py`.
...
Use Node.js 20 or newer for the TypeScript and Web UI tooling.
...
AWS administration scripts require configured AWS credentials and `scripts/.env` (copy `scripts/.env.example` and set the target `AWS_REGION`, `COGNITO_USER_POOL_ID`, and `DYNAMODB_TABLE_NAME`). The local conformance runner must remain independent of AWS `.env` configuration. AWS scripts may use `src/config.ts`; local-only scripts should not import it.
...
For deployed Web UI security-header or authentication changes, follow `docs/security/zap-scanning.md` and the manually triggered baseline/authenticated workflows under `.github/workflows/`. Keep these scans passive: the Web UI API multiplexes reads and mutations through one POST endpoint, so generic crawling or active scanning can alter sandbox data. Add a narrowly evidenced alert suppression only for a demonstrated false positive.
...
- [ ] The Confluence sync self-test and Python unit tests pass if its script or configuration changed; generated specification output was not hand-edited.
