...
* [3. Conformance Scenarios](#3-conformance-scenarios)
  ...
  * [3.1. AN Producer](#31-an-producer)

    * [3.1.1. AN Producer: GET scenarios for the required query parameter filter — Required](#311-an-producer-get-scenarios-for-the-required-query-parameter-filter--required)
    * [3.1.2. AN Producer: GET scenarios for optional query parameters — Optional/report-only](#312-an-producer-get-scenarios-for-optional-query-parameters--optionalreport-only)

      * [3.1.2.1. AN Producer: GET scenario for optional query parameter filters](#3121-an-producer-get-scenario-for-optional-query-parameter-filters)
      * [3.1.2.2. AN Producer: GET scenario for pagination](#3122-an-producer-get-scenario-for-pagination)
    * [3.1.3. AN Producer: POST scenarios for full arrival notices — Optional/report-only](#313-an-producer-post-scenarios-for-full-arrival-notices--optionalreport-only)
    * [3.1.4. AN Producer: POST scenario for arrival notice notifications — Optional/report-only](#314-an-producer-post-scenario-for-arrival-notice-notifications--optionalreport-only)
  * [3.2. AN Consumer](#32-an-consumer)

    * [3.2.1. AN Consumer: GET scenario — Required](#321-an-consumer-get-scenario--required)
    * [3.2.2. AN Consumer: POST scenarios — Optional/report-only](#322-an-consumer-post-scenarios--optionalreport-only)
* [4. Conformance Validations](#4-conformance-validations)

  * [4.1. Custom validations: AN Producer](#41-custom-validations-an-producer)

    * [4.1.1. Common arrival notice validations](#411-common-arrival-notice-validations)
    * [4.1.2. FREE_TIME scenario validations](#412-free_time-scenario-validations)
    * [4.1.3. FREIGHTED scenario validations](#413-freighted-scenario-validations)
    * [4.1.4. Arrival notice notification validations](#414-arrival-notice-notification-validations)
  * [4.2. Custom validations: AN Consumer](#42-custom-validations-an-consumer)
...
|---------------|-----------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------|
| AN Producer   | Ocean Carrier / Freight Forwarder | It is mandatory to support the following capabilities:<br><br>* Can make arrival notices available for retrieval through **GET** `/arrival-notices`                            | May additionally support the following capabilities:<br><br>* Can send full arrival notices by calling the **POST** `/arrival-notices` endpoint implemented by the AN Consumer<br>* Can send lightweight arrival notice notifications by calling the **POST** `/arrival-notice-notifications` endpoint implemented by the AN Consumer | * Basic<br>* Freighted<br>* Free time |
| AN Consumer   | Freight Forwarder / Shipper       | It is mandatory to support the following capabilities:<br><br>* Can retrieve arrival notices by calling the **GET** `/arrival-notices` endpoint implemented by the AN Producer | May additionally support the following capabilities:<br><br>* Can receive full arrival notices from the AN Producer through **POST** `/arrival-notices`<br>* Can receive lightweight arrival notice notifications from the AN Producer through **POST** `/arrival-notice-notifications`                                               | * Basic<br>* Freighted<br>* Free time |
...
FREIGHTED and FREE_TIME arrival notices must also satisfy the BASIC validations because they contain BASIC content in
...
* Supply parameters (`transportDocumentReferences`) + GET Arrival Notice (FREE_TIME)
...
For **FREE_TIME**, the adopter provides one or more transport document references such that the response contains at
...
The following custom validations apply to the BASIC, FREIGHTED, and FREE_TIME full-arrival-notice scenarios:
...
### 4.1.2. FREE_TIME scenario validations
