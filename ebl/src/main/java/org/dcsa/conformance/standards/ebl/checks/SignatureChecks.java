      ...
      delegate ->
          (nodeToValidate, contextPath) -> {
            ...
            try {
              ...
              return ConformanceCheckResult.simple(
                  Set.of(
                      "The path '%s' should have been a signed payload containing Json as content, but could not be parsed: %s"
                          .formatted(contextPath, e.toString())));
            }
  ...
  public static JsonContentMatchedValidation signatureValidates(
    ...
    return (nodeToValidate, contextPath) -> {
      ...
      try {
        ...
        return ConformanceCheckResult.simple(
            Set.of(
                "The path '%s' should have been a signed payload, but could not be parsed as a JWS (%s)."
                    .formatted(contextPath, e.toString())));
      }
      if (Algorithm.NONE.equals(jwsObject.getHeader().getAlgorithm())) {
        return ConformanceCheckResult.simple(
            Set.of(
                "The JWS payload at '%s' uses the 'none' algorithm and is therefore unsigned."
                    .formatted(contextPath)));
      }
      ...
      if (!signatureVerifier.verifySignature(jwsObject)) {
        return ConformanceCheckResult.simple(
            Set.of(
                "The path '%s' was a valid JWS, but it was not signed by the expected key"
                    .formatted(contextPath)));
      }
