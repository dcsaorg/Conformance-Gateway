...
class PintScenarioListBuilder extends ScenarioListBuilder<PintScenarioListBuilder> {
  ...
  private static final String TRANSFER_FINISHED_SIGNED_RESPONSE_SCHEMA =
      "EnvelopeTransferFinishedResponseSignedContent";

  private static final String TRANSFER_STARTED_UNSIGNED_RESPONSE_SCHEMA =
                                        ...
                                        initiateAndCloseTransferAction(PintResponseCode.RECE)
                                            .thenEither(
                                                ...
                                                retryTransfer(
                                                    ...
                                                    SenderTransmissionClass.VALID_TRANSFER),
                                                retryTransfer(
                                                    PintResponseCode.DUPE,
                                                    SenderTransmissionClass.VALID_TRANSFER,
                                                    RetryType.RESIGN),
                                                retryTransfer(
                                                    PintResponseCode.DISE,
                                                    SenderTransmissionClass.VALID_TRANSFER,
                                                    RetryType.MANIPULATE)),
                                        initiateAndCloseTransferAction(
                                            ...
                                            .thenEither(
                                                ...
                                                retryTransfer(
                                                    ...
                                                    RetryType.MANIPULATE)),
                                        initiateAndCloseTransferAction(
                                                PintResponseCode.BSIG,
                                                SenderTransmissionClass.SIGNATURE_ISSUE)
                                            .then(
                                                initiateAndCloseTransferAction(
                                                    PintResponseCode.RECE)),
                                        initiateAndCloseTransferAction(
                                            PintResponseCode.BENV,
                                            SenderTransmissionClass.WRONG_RECIPIENT_PLATFORM)),
                                receiverStateSetup(ScenarioClass.INVALID_RECIPIENT)
                                    .then(initiateAndCloseTransferAction(PintResponseCode.BENV))),
                        supplySenderTransferScenarioParameters(2)
                            .thenEither(
                                receiverStateSetup(ScenarioClass.NO_ISSUES)
                                    .then(
                                        initiateTransfer(2, SenderTransmissionClass.VALID_TRANSFER)
                                            .thenEither(
                                                transferDocument()
                                                    .thenEither(
                                                        transferDocument()
                                                                        ...
                                                                        transferDocument()
                                                                            .then(
                                                                                closeTransferAction(
                                                                                    PintResponseCode
                                                                                        .RECE)))),
                                                        closeTransferAction(PintResponseCode.MDOC)
                                                            .then(
                                                                retryTransfer(
                                                                        1,
                                                                        SenderTransmissionClass
  ...
  private PintScenarioListBuilder(Function<ConformanceAction, ConformanceAction> actionBuilder) {
    super(actionBuilder);
  }
    ...
    return new PintScenarioListBuilder(
        previousAction ->
            new SupplyValidationEndpointScenarioParametersAction(
                sendingPlatform, receivingPlatform, (PintAction) previousAction));
  }
    ...
    return new PintScenarioListBuilder(
        previousAction ->
            new SenderSupplyScenarioParametersAction(
                receivingPlatform, sendingPlatform, (PintAction) previousAction, documentCount));
  }
  ...
  private static PintScenarioListBuilder receiverStateSetup(ScenarioClass scenarioClass) {
    ...
    return new PintScenarioListBuilder(
        previousAction ->
            new ReceiverSupplyScenarioParametersAndStateSetupAction(
                receivingPlatform, sendingPlatform, (PintAction) previousAction, scenarioClass));
  }
  ...
  private static PintScenarioListBuilder transferDocument(
    ...
    return new PintScenarioListBuilder(
        previousAction ->
            new PintTransferAdditionalDocumentAction(
                receivingPlatform,
                sendingPlatform,
                (PintAction) previousAction,
                senderDocumentTransmissionTypeCode,
  ...
  private static PintScenarioListBuilder initiateTransfer(
      int expectedMissingDocumentCount, SenderTransmissionClass senderTransmissionClass) {
    String sendingPlatform = SENDING_PLATFORM_PARTY_NAME.get();
    String receivingPlatform = RECEIVING_PLATFORM_PARTY_NAME.get();
    return new PintScenarioListBuilder(
        previousAction ->
  ...
  private static PintScenarioListBuilder retryTransfer(
      PintResponseCode pintResponseCode, SenderTransmissionClass senderTransmissionClass) {
  ...
  private static PintScenarioListBuilder initiateAndCloseTransferAction(
      PintResponseCode signedResponseCode) {
    return initiateAndCloseTransferAction(
        signedResponseCode, SenderTransmissionClass.VALID_ISSUANCE);
  }

  private static PintScenarioListBuilder initiateAndCloseTransferAction(
      PintResponseCode signedResponseCode, SenderTransmissionClass senderTransmissionClass) {
    String sendingPlatform = SENDING_PLATFORM_PARTY_NAME.get();
    String receivingPlatform = RECEIVING_PLATFORM_PARTY_NAME.get();
    return new PintScenarioListBuilder(
        previousAction ->
