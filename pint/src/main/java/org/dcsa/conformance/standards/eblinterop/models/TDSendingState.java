...
public class TDSendingState {
  ...
  @SneakyThrows
  public void manipulateLatestTransaction(
      PayloadSigner payloadSigner, ReceiverScenarioParameters rsp) {
    var chain = this.state.path(ENVELOPE_TRANSFER_CHAIN);
    var lastSigned = chain.path(chain.size() - 1);
  ...
  public ObjectNode generateEnvelopeManifest(
      String transportDocumentChecksum,
      String lastEnvelopeTransferChainEntrySignedContentChecksum) {
    ...
    for (var documentNode : this.state.path(DOCUMENTS)) {
      ...
      docNode
          .put("name", "test-document-" + (++docNo) + ".bin")
