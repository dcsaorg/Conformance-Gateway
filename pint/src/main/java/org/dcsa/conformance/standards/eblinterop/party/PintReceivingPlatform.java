...
public class PintReceivingPlatform extends ConformanceParty {
  ...
  private static final PayloadSignerWithKey RECEIVING_PLATFORM_PAYLOAD_SIGNER =
  ...
  protected Map<Class<? extends ConformanceAction>, Consumer<JsonNode>> getActionPromptHandlers() {
    return Map.ofEntries(
        ...
        Map.entry(
            SupplyValidationEndpointScenarioParametersAction.class,
            this::providedReceiverValidationScenarioParameters));
  }

  private void providedReceiverValidationScenarioParameters(JsonNode actionPrompt) {
    log.info(
        "EblInteropReceivingPlatform.providedReceiverValidationScenarioParameters(%s)"
            .formatted(actionPrompt.toPrettyString()));
    asyncOrchestratorPostPartyInput(
        actionPrompt.required("actionId").asText(),
        SupplyValidationEndpointScenarioParametersAction.getJsonForPrompt());
    addOperatorLogEntry("Finished providedReceiverValidationScenarioParameters");
  }

  private void initiateState(JsonNode actionPrompt) {
    log.info(
        "EblInteropSendingPlatform.handleScenarioTypeAction(%s)"
            .formatted(actionPrompt.toPrettyString()));
    ...
    var tdState =
        TDReceiveState.newInstance(
            tdr, ssp.sendersX509SigningCertificateInPEMFormat(), receivingParameters);
    tdState.setExpectedReceiver(
        generateReceiverParty(
            ssp.eblPlatform(),
            scenarioClass == ScenarioClass.INVALID_RECIPIENT
                ? ScenarioClass.NO_ISSUES
                : scenarioClass,
    ...
    addOperatorLogEntry("Prompt answer for initiateState: %s".formatted(receivingParametersJson));
    asyncOrchestratorPostPartyInput(
        actionPrompt.required("actionId").asText(), receivingParametersJson);
    addOperatorLogEntry("Finished ScenarioType setup");
  }

  private static JsonNode generateReceiverParty(
      String sendingPlatform, ScenarioClass scenarioClass, String expectedRecipient) {
    String platform;
  ...
  public static ReceiverScenarioParameters getReceiverScenarioParameters(
      String sendingPlatform, ScenarioClass scenarioClass) {
    var receiverParty =
        generateReceiverParty(sendingPlatform, scenarioClass, EXPECTED_RECEIVER_PARTY_CODE);

    return new ReceiverScenarioParameters(
        receiverParty, RECEIVING_PLATFORM_PAYLOAD_SIGNER.getPublicKeyInPemFormat());
  }
  ...
  public ConformanceResponse handleInitiateTransferRequest(ConformanceRequest request) {
    ...
    var lastEnvelopeTransferChainEntrySignedContentChecksum =
        Checksums.sha256(lastEtcEntry.asText(""));
    var unsignedPayload =
        OBJECT_MAPPER
            .createObjectNode()
            .put(
                ...
                lastEnvelopeTransferChainEntrySignedContentChecksum);
    var responseCode =
        receiveState.recommendedFinishTransferResponse(
            transferRequest, receiveState.getSignatureVerifierForSenderSignatures());

    if (responseCode != null) {
      receiveState.updateTransferState(responseCode);
      unsignedPayload.put("responseCode", responseCode.name());
      var signedPayloadJsonNode =
          receiveState.generateSignedResponse(responseCode, RECEIVING_PLATFORM_PAYLOAD_SIGNER);
  ...
  protected void exportPartyJsonState(ObjectNode targetObjectNode) {
    targetObjectNode.set("envelopeReferences", StateManagementUtil.storeMap(envelopeReferences));
  }
  ...
  private static final Pattern ENVELOPE_ADDITIONAL_DOCUMENT_PATTERN =
      Pattern.compile("/envelopes/([^/]++)/additional-documents/([0-9a-fA-F]+)/?$");

  private static final Pattern FINISH_TRANSFER_PATTERN =
      Pattern.compile("/envelopes/([^/]++)/finish-transfer/?$");
  ...
  public ConformanceResponse handleEnvelopeRequest(ConformanceRequest request) {
    ...
    if (eadm.find()) {
      ...
      if (tdr != null) {
        ...
        if (!Objects.equals(checksum, computedChecksum)
          ...
          var payload =
              receiveState.generateSignedResponse(INCD, RECEIVING_PLATFORM_PAYLOAD_SIGNER);
          return request.createResponse(
        ...
        receiveState.save(persistentMap);
        return request.createResponse(
            204, Map.of(API_VERSION, List.of(apiVersion)), new ConformanceMessageBody(""));
      }
      return request.createResponse(
          404,
          Map.of(API_VERSION, List.of(apiVersion)),
          new ConformanceMessageBody(
              OBJECT_MAPPER.createObjectNode().put("message", "Unknown envelope reference")));
    }
    ...
    return request.createResponse(
        ...
        Map.of(API_VERSION, List.of(apiVersion)),
        new ConformanceMessageBody(
            OBJECT_MAPPER.createObjectNode().put("message", "Unknown endpoint")));
  }
  ...
  private ConformanceResponse handleReceiverValidation(ConformanceRequest request) {
    return request.createResponse(
        ...
        Map.of(API_VERSION, List.of(apiVersion)),
        new ConformanceMessageBody(
            OBJECT_MAPPER.createObjectNode().put("partyName", "Name of Test Party")));
  }
