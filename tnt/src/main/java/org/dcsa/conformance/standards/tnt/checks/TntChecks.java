...
public class TntChecks {
            ...
            body -> {
              ...
              if (eventCreatedDateTimeParam.isPresent()) {
                String[] eventCreatedDateTimeParamKey =
                    eventCreatedDateTimeParam.get().getKey().getQueryParamName().split(":");
                    ...
                    filterNodesByEventType(body, SHIPMENT_EVENT_TYPE)
                        .filter(
                            node -> {
                              ...
                              return !(shipmentEventTypeCodeNode.isMissingNode()
                                      && shipmentEventTypeCodeNode.isNull())
                                  && !expectedShipmentEventTypeCodes.contains(
                                      shipmentEventTypeCodeNode.asText())
                                  && !VALID_SHIPMENT_EVENT_TYPES.contains(
                                      shipmentEventTypeCodeNode.asText());
                            })
                        .map(
                            node ->
                                "Missing or mismatched shipmentEventTypeCode "
                                    + "for eventType SHIPMENT at "
                    ...
                    Stream.concat(
                        ...
                        .filter(
                            node -> {
                              ...
                              return !(documentTypeCodeNode.isMissingNode()
                                      || documentTypeCodeNode.isNull())
                                  && !expectedDocumentTypeCodes.contains(
                                      documentTypeCodeNode.asText())
                                  && !VALID_DOCUMENT_TYPE_CODES.contains(
                                      documentTypeCodeNode.asText());
                            })
                        .map(
                            node ->
              ...
              if (transportEventTypeCodeParam.isPresent()) {
                ...
                Set<String> errors =
                    filterNodesByEventType(body, TRANSPORT_EVENT_TYPE)
                        ...
                        .map(
                            node ->
                                "Missing or mismatched transportEventTypeCode for eventType TRANSPORT at "
                                    + node.path("eventId").asText())
                        .collect(Collectors.toSet());
                    ...
                    filterNodesByEventType(body, EQUIPMENT_EVENT_TYPE)
                        .filter(
                            node -> {
                              ...
                              return equipmentEventTypeCodeNode.isMissingNode()
                                  || equipmentEventTypeCodeNode.isNull()
                                  || !expectedEquipmentEventTypeCodes.contains(
                                      equipmentEventTypeCodeNode.asText())
                                  || !VALID_EQUIPMENT_EVENT_TYPES.contains(
                                      equipmentEventTypeCodeNode.asText());
                            })
                        .map(
                            node ->
                                "Missing or mismatched equipmentEventTypeCode for eventType EQUIPMENT at "
                                    + node.path("eventId").asText())
                        .collect(Collectors.toSet());
            ...
            body -> {
              ...
              if (limitParam.isPresent()) {
                ...
                if (body.size() > expectedLimit) {
                  return Set.of(
                      "The number of events exceeds the limit parameter: " + expectedLimit);
                }
              }
          ...
          filterNodesByEventType(body, EQUIPMENT_EVENT_TYPE)
              .filter(
                  node -> {
                    ...
                    return !(equipmentReferenceNode.isMissingNode()
                            || equipmentReferenceNode.isNull())
                        && !expectedEquipmentReferences.contains(equipmentReferenceNode.asText());
                  })
              .map(
                  node ->
                      "Missing or mismatched equipmentReference for eventType EQUIPMENT at "
                          + node.path("eventId").asText())
              .collect(Collectors.toSet()));
  ...
  private Set<String> validateCarrierBookingReference(
      JsonNode body, Supplier<SuppliedScenarioParameters> sspSupplier) {
    ...
    if (carrierBookingReferenceParam.isPresent()) {
          ...
          Stream.concat(
              ...
              .filter(
                  node -> {
                    ...
                    boolean isValid =
                        StreamSupport.stream(documentReferencesNode.spliterator(), false)
                            .anyMatch(
                                refNode ->
                                    !(refNode.path("documentReferenceType").asText().equals("BKG")
              ...
              .map(
                  node ->
                      "Missing or mismatched carrierBookingReference for eventType "
                          + node.path("eventType").asText()
                          + " at "
                          + node.path("eventId").asText())
              .collect(Collectors.toSet());
      ...
      errors.addAll(
          filterNodesByEventType(body, SHIPMENT_EVENT_TYPE)
              ...
              .map(
                  node ->
                      "Missing or mismatched carrierBookingReference for eventType SHIPMENT at "
                          + node.path("eventId").asText())
              .collect(Collectors.toSet()));
