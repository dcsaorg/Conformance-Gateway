...
public class TntChecks {
  ...
  public static ActionCheck responseContentChecks(UUID matched, String standardVersion, Supplier<SuppliedScenarioParameters> sspSupplier) {
    ...
    checks.add(JsonAttribute.customValidator(
      ...
      body -> {
        Set<OffsetDateTime> eventCreatedDateTimes = StreamSupport.stream(body.spliterator(), false)
          ...
          .collect(Collectors.toSet());
        Optional<Map.Entry<TntFilterParameter, String>> eventCreatedDateTimeParam = sspSupplier.get().getMap().entrySet().stream()
          .filter(e -> e.getKey().equals(TntFilterParameter.EVENT_CREATED_DATE_TIME))
          .findFirst();
        if (eventCreatedDateTimeParam.isPresent()) {
          String[] eventCreatedDateTimeParamKey = eventCreatedDateTimeParam.get().getKey().getQueryParamName().split(":");
          OffsetDateTime eventCreatedDateTimeParamValue = OffsetDateTime.parse(eventCreatedDateTimeParam.get().getValue());
          if (eventCreatedDateTimeParamKey.length == 2) {
            return validateEventCreatedDateTime(eventCreatedDateTimes, eventCreatedDateTimeParamKey, eventCreatedDateTimeParamValue);
          }
        }
        return Set.of();
      }
    ...
    checks.add(JsonAttribute.customValidator(
      "Validate shipmentEventTypeCode exists and matches in JSON response if request parameter has shipmentEventTypeCode",
      body -> {
        Optional<Map.Entry<TntFilterParameter, String>> shipmentEventTypeCodeParam = sspSupplier.get().getMap().entrySet().stream()
          .filter(e -> e.getKey().equals(TntFilterParameter.SHIPMENT_EVENT_TYPE_CODE))
          .findFirst();

        if (shipmentEventTypeCodeParam.isPresent()) {
          Set<String> expectedShipmentEventTypeCodes = Arrays.stream(shipmentEventTypeCodeParam.get().getValue().split(","))
            .map(String::trim)
            .collect(Collectors.toSet());
          ...
          Set<String> errors = filterNodesByEventType(body, SHIPMENT_EVENT_TYPE)
            ...
            .map(node -> "Missing or mismatched shipmentEventTypeCode for eventType SHIPMENT at " + node.path("eventId").asText())
            .collect(Collectors.toSet());
    ...
    checks.add(JsonAttribute.customValidator(
      ...
      body -> {
        Optional<Map.Entry<TntFilterParameter, String>> documentTypeCodeParam = sspSupplier.get().getMap().entrySet().stream()
        ...
        if (documentTypeCodeParam.isPresent()) {
          ...
          Set<String> errors = Stream.concat(
            ...
            .map(node -> "Missing or mismatched documentTypeCode for eventType " + node.path("eventType").asText() + " at " + node.path("eventId").asText())
            .collect(Collectors.toSet());
    ...
    checks.add(JsonAttribute.customValidator(
      "Validate transportEventTypeCode exists and matches in JSON response if request parameter has transportEventTypeCode",
      body -> {
        Optional<Map.Entry<TntFilterParameter, String>> transportEventTypeCodeParam = sspSupplier.get().getMap().entrySet().stream()
          .filter(e -> e.getKey().equals(TntFilterParameter.TRANSPORT_EVENT_TYPE_CODE))
          .findFirst();

        if (transportEventTypeCodeParam.isPresent()) {
          Set<String> expectedTransportEventTypeCodes = Arrays.stream(transportEventTypeCodeParam.get().getValue().split(","))
            .map(String::trim)
            .collect(Collectors.toSet());
          ...
          Set<String> errors = filterNodesByEventType(body, "TRANSPORT")
            .filter(node -> {
              ...
              return transportEventTypeCodeNode.isMissingNode() || transportEventTypeCodeNode.isNull() || !expectedTransportEventTypeCodes.contains(transportEventTypeCodeNode.asText());
            })
            .map(node -> "Missing or mismatched transportEventTypeCode for eventType TRANSPORT at " + node.path("eventId").asText())
            .collect(Collectors.toSet());
    ...
    checks.add(JsonAttribute.customValidator(
      ...
      body -> validateParameter(body, sspSupplier, TntFilterParameter.TRANSPORT_CALL_ID, "/transportCall/transportCallID", "TRANSPORT", "EQUIPMENT")
    ));

    checks.add(JsonAttribute.customValidator(
      "Validate vesselIMONumber exists and matches in JSON response if request parameter has vesselIMONumber",
      body -> validateParameter(body, sspSupplier, TntFilterParameter.VESSEL_IMO_NUMBER, "/transportCall/vessel/vesselIMONumber", "TRANSPORT", "EQUIPMENT")
    ));

    checks.add(JsonAttribute.customValidator(
      "Validate exportVoyageNumber exists and matches in JSON response if request parameter has exportVoyageNumber",
      body -> validateParameter(body, sspSupplier, TntFilterParameter.EXPORT_VOYAGE_NUMBER, "/transportCall/exportVoyageNumber", "TRANSPORT", "EQUIPMENT")
    ));

    checks.add(JsonAttribute.customValidator(
      "Validate carrierServiceCode exists and matches in JSON response if request parameter has carrierServiceCode",
      body -> validateParameter(body, sspSupplier, TntFilterParameter.CARRIER_SERVICE_CODE, "/transportCall/carrierServiceCode", "TRANSPORT", "EQUIPMENT")
    ));

    checks.add(JsonAttribute.customValidator(
      "Validate UNLocationCode exists and matches in JSON response if request parameter has UNLocationCode",
      body -> validateParameter(body, sspSupplier, TntFilterParameter.UN_LOCATION_CODE, "/transportCall/location/UNLocationCode", "TRANSPORT", "EQUIPMENT")
    ));

    checks.add(JsonAttribute.customValidator(
      "Validate equipmentEventTypeCode exists and matches in JSON response if request parameter has equipmentEventTypeCode",
      body -> {
        Optional<Map.Entry<TntFilterParameter, String>> equipmentEventTypeCodeParam = sspSupplier.get().getMap().entrySet().stream()
          .filter(e -> e.getKey().equals(TntFilterParameter.EQUIPMENT_EVENT_TYPE_CODE))
          .findFirst();

        if (equipmentEventTypeCodeParam.isPresent()) {
          Set<String> expectedEquipmentEventTypeCodes = Arrays.stream(equipmentEventTypeCodeParam.get().getValue().split(","))
            .map(String::trim)
            .collect(Collectors.toSet());
          ...
          Set<String> errors = filterNodesByEventType(body, "EQUIPMENT")
            ...
            .map(node -> "Missing or mismatched equipmentEventTypeCode for eventType EQUIPMENT at " + node.path("eventId").asText())
            .collect(Collectors.toSet());
    ...
    checks.add(JsonAttribute.customValidator(
      "Validate equipmentReference exists and matches in JSON response if request parameter has equipmentReference",
      body -> validateEquipmentReference(body, sspSupplier)
    ));

    checks.add(JsonAttribute.customValidator(
      "Validate carrierBookingReference exists and matches in JSON response if request parameter has carrierBookingReference",
      body -> validateCarrierBookingReference(body, sspSupplier)
    ));
  ...
  private Set<String> validateParameter(JsonNode body, Supplier<SuppliedScenarioParameters> sspSupplier, TntFilterParameter parameter, String jsonPath, String eventType1, String eventType2) {
    ...
    if (param.isPresent()) {
      ...
      Set<String> errors = Stream.concat(
        ...
        .filter(node -> {
          ...
          return valueNode.isMissingNode() || valueNode.isNull() || !expectedValues.contains(valueNode.asText());
        })
        .map(node -> "Missing or mismatched " + jsonPath + " for eventType " + node.path("eventType").asText() + " at " + node.path("eventId").asText())
        .collect(Collectors.toSet());
  ...
  private Set<String> validateEquipmentReference(JsonNode body, Supplier<SuppliedScenarioParameters> sspSupplier) {
    Optional<Map.Entry<TntFilterParameter, String>> equipmentReferenceParam = sspSupplier.get().getMap().entrySet().stream()
    ...
    if (equipmentReferenceParam.isPresent()) {
      ...
      Set<String> errors = Stream.concat(
        ...
        .map(node -> "Missing or mismatched equipmentReference for eventType " + node.path("eventType").asText() + " at " + node.path("eventId").asText())
        .collect(Collectors.toSet());
      ...
      errors.addAll(
        filterNodesByEventType(body, "EQUIPMENT")
          .filter(node -> {
            ...
            return equipmentReferenceNode.isMissingNode() || equipmentReferenceNode.isNull() || !expectedEquipmentReferences.contains(equipmentReferenceNode.asText());
          })
          .map(node -> "Missing or mismatched equipmentReference for eventType EQUIPMENT at " + node.path("eventId").asText())
          .collect(Collectors.toSet())
      );
  ...
  private Set<String> validateCarrierBookingReference(JsonNode body, Supplier<SuppliedScenarioParameters> sspSupplier) {
    Optional<Map.Entry<TntFilterParameter, String>> carrierBookingReferenceParam = sspSupplier.get().getMap().entrySet().stream()
      .filter(e -> e.getKey().equals(TntFilterParameter.CARRIER_BOOKING_REFERENCE))
      .findFirst();

    if (carrierBookingReferenceParam.isPresent()) {
      Set<String> expectedCarrierBookingReferences = Arrays.stream(carrierBookingReferenceParam.get().getValue().split(","))
        .map(String::trim)
        .collect(Collectors.toSet());
      ...
      Set<String> errors = Stream.concat(
        ...
        .map(node -> "Missing or mismatched carrierBookingReference for eventType " + node.path("eventType").asText() + " at " + node.path("eventId").asText())
        .collect(Collectors.toSet());
      ...
      errors.addAll(
        filterNodesByEventType(body, SHIPMENT_EVENT_TYPE)
          .filter(node -> {
            ...
            return !documentTypeCodeNode.asText().equals("BKG") || !expectedCarrierBookingReferences.contains(documentIDNode.asText());
          })
          .map(node -> "Missing or mismatched carrierBookingReference for eventType SHIPMENT at " + node.path("eventId").asText())
          .collect(Collectors.toSet())
      );
  ...
  private Set<String> validateEventCreatedDateTime(Set<OffsetDateTime> eventCreatedDateTimes, String[] eventCreatedDateTimeParamKey, OffsetDateTime eventCreatedDateTimeParamValue) {
    switch (eventCreatedDateTimeParamKey[1]) {
      case "gte":
        if (eventCreatedDateTimes.stream().anyMatch(dateTime -> dateTime.isBefore(eventCreatedDateTimeParamValue))) {
          return Set.of("Event Created Date Time is not greater than or equal to " + eventCreatedDateTimeParamValue);
        }
        break;
      case "gt":
        if (eventCreatedDateTimes.stream().anyMatch(dateTime -> dateTime.compareTo(eventCreatedDateTimeParamValue) <= 0)) {
